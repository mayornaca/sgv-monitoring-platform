                ...
                onChange: function onChange(data, state) {
                    ...
                    function parseData(rawData) {
                        ...
                        for (var i = 0, ilen = rawData.length; i < ilen; i++) {
                            var group = rawData[i].g;
                            state.completeStructData.push({
                                group: group,
                                lines: rawData[i].d.map(function (d) {
                                    return d.l;
                                })
                            });

                            for (var j = 0, jlen = rawData[i].d.length; j < jlen; j++) {
                                for (var k = 0, klen = rawData[i].d[j].d.length; k < klen; k++) {
                                    var _rawData$i$data$j$dat = rawData[i].d[j].d[k],
                                        timeRange = _rawData$i$data$j$dat.t,
                                        val = _rawData$i$data$j$dat.v,
                                        labelVal = _rawData$i$data$j$dat.labelVal;
                                    state.completeFlatData.push({
                                        group: group,
                                        label: rawData[i].d[j].l,
                                        timeRange: timeRange.map(function (d) {
                                            return new Date(d*1000);
                                        }),
                                        val: val,
                                        labelVal: labelVal !== undefined ? labelVal : val,
                                        data: rawData[i].d[j].d[k]
                                    });
                                }
        ...
        init: function init(el, state) {
            ...
            function addTooltips() {
                ...
                state.segmentTooltip = d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (d) {
                    ...
                    return  '<strong>Desde: </strong>' + dateFormat(d.timeRange[0]) + '<br>' + '<strong>Hasta: </strong>' + dateFormat(d.timeRange[1]);
                });
