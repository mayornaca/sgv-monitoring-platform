        ...
        onChange: function onChange(data, state) {
          ...
          function parseData(rawData) {
            ...
            for (var i = 0, ilen = rawData.length; i < ilen; i++) {
              // Support both full format (group, data, label, timeRange, val) and abbreviated format (g, d, l, t, v)
              var group = rawData[i].group || rawData[i].g;
              var groupData = rawData[i].data || rawData[i].d;
              state.completeStructData.push({
                group: group,
                lines: groupData.map(function (d) {
                  return d.label || d.l;
                })
              });
              for (var j = 0, jlen = groupData.length; j < jlen; j++) {
                var lineData = groupData[j].data || groupData[j].d;
                for (var k = 0, klen = lineData.length; k < klen; k++) {
                  var segmentData = lineData[k];
                  var timeRange = segmentData.timeRange || segmentData.t;
                  var val = segmentData.val !== undefined ? segmentData.val : segmentData.v;
                  var labelVal = segmentData.labelVal !== undefined ? segmentData.labelVal : val;
                  // Skip segments with empty timeRange arrays
                  if (!timeRange || !Array.isArray(timeRange) || timeRange.length === 0) {
                    continue;
                  }
                  state.completeFlatData.push({
                    group: group,
                    label: groupData[j].label || groupData[j].l,
